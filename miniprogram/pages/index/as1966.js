Page({
  ...
  data: {
    ...
    linchuang: [
      { id: 0, desc: "腰椎前屈、后伸、侧弯3个方向活动受限" },
      { id: 1, desc: "腰背痛病史或现在症"},
      { id: 2, desc: "第4肋间隙测量胸廓活动度<2.5cm"}
    ],
    diqia_left: [
      { id: 3, desc: "0级（正常）" },
      { id: 4, desc: "1级（可疑）" },
  ...
  compute: function() {
    ...
    var conclusion = "";
    if ( left_score >= 3 && right_score >=3 && linchuang_num >= 1 ){
      conclusion = "肯定AS";
    }
    //单侧3-4级或双侧2级骶髂关节炎伴第1项或2+3项临床标准
    //-----start-----
    else if (left_score >= 3 && linchuang[0].checked){
      conclusion = "肯定AS";
    }
    else if (left_score >= 3 && linchuang[1].checked && linchuang[2].checked ){
      conclusion = "肯定AS";
    }
    else if (right_score >= 3 && linchuang[0].checked) {
      conclusion = "肯定AS";
    }
    else if (right_score >= 3 && linchuang[1].checked && linchuang[2].checked) {
      conclusion = "肯定AS";
    }
    else if (left_score == 2 && right_score == 2 && linchuang[0].checked)
    {
      conclusion = "肯定AS";
    }
    else if (left_score == 2 && right_score == 2 && linchuang[1].checked && linchuang[2].checked){
      conclusion = "肯定AS";
    }
    //-----end-----
    else if ( left_score >= 3 && right_score >= 3 && linchuang_num == 0 ){
      conclusion = "可能AS";
    }
    else{
      conclusion = "无法诊断";
